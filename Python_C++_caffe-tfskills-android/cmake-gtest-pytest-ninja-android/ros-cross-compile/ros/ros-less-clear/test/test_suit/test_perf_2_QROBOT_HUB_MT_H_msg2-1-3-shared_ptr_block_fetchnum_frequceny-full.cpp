  ...
  std::cout<<"\n" <<" TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg->frameid["<<msg1->frameid<<"]" << std::endl;
   ...
   void operator() () {
	...
	if (TASK_NAME=="THREAD1") {
		...
		mhub.subscribe<std::shared_ptr<qrobot_msg_type3>>(TASK_NAME,"topic3",subs_queue_size,0,std::bind(&algo_task::callback_1<std::shared_ptr<qrobot_msg_type3>>,this,std::placeholders::_1),error);
...
`
	...
	std::thread sub1([&](){
		std::cout <<"\n" <<" TASK_NAME("<< TASK_NAME<<")  sub1 thread function\n";
		std::string error("   ok           ");
                int i=1;
                while (i<runs_s) {
			  	if (TASK_NAME=="THREAD1") {
					...
					mhub.spinOne<std::shared_ptr<qrobot_msg_type3>>(TASK_NAME,"topic3",fetchnum,isblock,error);
				 ...
				 if (TASK_NAME=="THREAD3") {
                                        ...
                                        spinonce_callback_2_hist.push_back(spinonce_callback_2);
                                        spinonce_callback_2=0;
					mhub.spinOne<std::shared_ptr<qrobot_msg_type1>>(TASK_NAME,"topic1",fetchnum,isblock,error);
				...
				if (TASK_NAME=="THREAD2") {
                	...
                	std::cout<<"\n"  << "TASK_NAME("<< TASK_NAME<<")loop in sub1 Task(i="<<i<<"   "<< TASK_NAME<< std::endl;
        		//std::this_thread::yield();
	 ...
	 std::thread sub2([&](){
                std::cout <<"\n" <<" TASK_NAME("<< TASK_NAME<<")  sub2 thread function\n";
                std::string error("   ok           ");
                int i=1;
                while (i<runs_s) {
                                if (TASK_NAME=="THREAD1") {
                        ...
                        std::cout<<"\n"  << "TASK_NAME("<< TASK_NAME<<")loop in sub2 Task(i="<<i<<"   "<< TASK_NAME<< std::endl;
                        //std::this_thread::yield();
                        std::this_thread::sleep_for(_duration_s);
                        i=i+1;
                };
	 ...
	 std::thread sub3([&](){
                std::cout <<"\n" <<" TASK_NAME("<< TASK_NAME<<")  sub3 thread function\n";
                std::string error("   ok           ");
                int i=1;
                while (i<runs_s) {
                                if (TASK_NAME=="THREAD1") {
                        ...
                        std::cout<<"\n"  << "TASK_NAME("<< TASK_NAME<<")loop in sub3 Task(i="<<i<<"   "<< TASK_NAME<< std::endl;
                        //std::this_thread::yield();
                        std::this_thread::sleep_for(_duration_s);
                        i=i+1;
                };
	...
	sub1.join();
	std::cout <<"\n" <<" TASK_NAME("<< TASK_NAME<<")  sub1 join done\n";
	sub2.join();
        std::cout <<"\n" <<" TASK_NAME("<< TASK_NAME<<")  sub2 join done\n";
...
bool miss_check(){
	 ...
	 if (TASK_NAME=="THREAD1") {
		...
		if (g_algo2_out.size()==algo_in_2.size() && g_algo1_out.size()==algo_in_1.size()) return true;
         };


	if (TASK_NAME=="THREAD2") {

                if (g_algo1_out.size()==algo_in_1.size() &&  g_algo2_out.size()==algo_in_2.size()) return true;
         };
...
int test_receiver_faster_block_fetch_1_1() {
	algo_task  m_algo_task1("THREAD1",12,1,1000,1000,1,1,1,10,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",12,1,1000,1000,1,1,1,10,100,100);
        std::thread     prod2(m_algo_task2);


	 std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();

	 std::cout<<"\n=================2222222222222====================" << std::endl;
        prod2.join();
...
int test_sender_faster_block_fetch_1_1() {
        algo_task  m_algo_task1("THREAD1",1,12,1000,1000,1,1,1,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",1,12,1000,1000,1,1,1,1000,100,100);
        std::thread     prod2(m_algo_task2);
...
int test_onefaster_oneslow_block_fetch_1_1() {

	algo_task  m_algo_task1("THREAD1",1,12,1000,1000,1,1,1,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",12,1,1000,1000,1,1,1,1000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_receiver_faster_block_fetch_0_0() {
        algo_task  m_algo_task1("THREAD1",12,1,1000,12000,1,0,0,10,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",12,1,1000,12000,1,0,0,10,100,100);
        std::thread     prod2(m_algo_task2);
...
int test_sender_faster_block_fetch_0_0() {
        algo_task  m_algo_task1("THREAD1",1,12,1000,12000,1,0,0,10,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",1,12,1000,12000,1,0,0,10,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_onefaster_oneslow_block_fetch_0_0() {
        algo_task  m_algo_task1("THREAD1",1,3,1000,3000,1,0,0,3000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",3,1,1000,3000,1,0,0,3000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_receiver_faster_block_fetch_0_1_mix() {
        algo_task  m_algo_task1("THREAD1",3,1,1000,4000,1,0,0,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",3,1,1000,1000,1,1,1,1000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_receiver_faster_block_fetch_1_0_mix() {
	algo_task  m_algo_task1("THREAD1",3,1,1000,1000,1,1,1,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",3,1,1000,4000,1,0,0,1000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_sender_faster_block_fetch_0_1_mix() {
        algo_task  m_algo_task1("THREAD1",1,3,1000,4000,1,0,0,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",1,3,1000,1000,1,1,1,1000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
int test_sender_faster_block_fetch_1_0_mix() {
        algo_task  m_algo_task1("THREAD1",1,3,1000,1000,1,1,1,1000,100,100);
        std::thread     prod1(m_algo_task1);
        algo_task  m_algo_task2("THREAD2",1,3,1000,4000,1,0,0,1000,100,100);
        std::thread     prod2(m_algo_task2);
        std::cout<<"\n=================1111111111111====================" << std::endl;
        prod1.join();
        std::cout<<"\n=================2222222222222====================" << std::endl;
...
//=======================re-factactor( re-use)   algo 2X2 th two componets connection  test============================================================
	//test_receiver_faster_block_fetch_1_1(); // pass
        // test_sender_faster_block_fetch_1_1(); //pass
        // test_onefaster_oneslow_block_fetch_1_1(); //pass

	//test_receiver_faster_block_fetch_0_0();  // pass 
        //test_sender_faster_block_fetch_0_0();//pass
	//test_onefaster_oneslow_block_fetch_0_0();a//pass

         //test_receiver_faster_block_fetch_0_1_mix();//pass 
	//test_receiver_faster_block_fetch_1_0_mix();//pass
	//test_sender_faster_block_fetch_0_1_mix(); //pass
	//test_sender_faster_block_fetch_1_0_mix();//pass
