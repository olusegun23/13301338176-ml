...
g++ -I/tmp/eigen -O2 example.cpp -o example
```

* Packaged by all Linux distributions, MacOS, Windows
* Opensource: MPL2

## Multi-platforms

* Supported compilers: GCC (≥4.2), MSVC (≥2008), Intel ICC, Clang/LLVM
* Supported systems: 
...
* other modules (**autodiff**, non-linear opt., FFT, etc.)
...
```c++
typedef Matrix<mpreal,Dynamic,Dynamic> MatrixMP; MatrixMP A, B, X;
// init A and B
...
``` 
`Eigen's API`   `Existing Other libs/apps`
      |                  |
      |            `BLAS/Lapack API`
      |               /
   Eigen's algorithms
...
* Expression templates
  ...
  - build an expression tree
   ...
   <img src = "https://github.com/PaddlePaddle/Paddle/wiki/refactor_discuess/expression.png" width=300>
...
* Immediate question: How to evaluate this?
  ...
  - Evaluation of expressions: top-down creation of an evaluator
 ...
 Conclusion: m3 = m1 + m2 + m3; 

   <img src = "https://github.com/PaddlePaddle/Paddle/wiki/refactor_discuess/eva_expression.png" width=600>
...
### `Tensor<data_type, rank>`

`Tensor<data_type, rank>` is the tensor and owns the memory.  `data_type` could be float or int, and `rank` is the number of dimensions, For example, rank 2 means a matrix.

If you assign a tensor A to B, B would be resized if necessary to match A.

#### The Constructor

`Tensor<data_type, rank>(size0, size1, ...) `rank` sizes.
