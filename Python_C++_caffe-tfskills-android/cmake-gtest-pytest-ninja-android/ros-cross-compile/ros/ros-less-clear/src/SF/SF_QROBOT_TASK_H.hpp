#ifndef SF_QROBOT_TASK_H
#define SF_QROBOT_TASK_H
...
class  algo_sf:public qrobot_task {
public:
	algo_sf(std::string str):TASK_NAME(str){};

	template<typename T>
	void callback_sf_vslam2(T msg1) {
	...
	void operator() () {
		...
		std::string error("   ok           ");
		mhub.advertise<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"SF",2,0,error); 
 		std::function<void(std::shared_ptr<qrobot_msg_type1>)> f1=std::bind(&algo_sf::callback_sf_vslam2<std::shared_ptr<qrobot_msg_type1>>,this,std::placeholders::_1);
		mhub.subscribe<std::shared_ptr<qrobot_msg_type1>>(TASK_NAME,"VSLAM2",g_subs_queue_size,0,f1,error);
	...
	//======================feed dynamic vector of vector data========================================
			...
			mhub.publish<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"SF",msg2,0,error);

			if (i>g_fetchnum){ 
				mhub.spinOne<std::shared_ptr<qrobot_msg_type1>>(TASK_NAME,"VSLAM2",g_fetchnum,g_isblock,error);
			};
