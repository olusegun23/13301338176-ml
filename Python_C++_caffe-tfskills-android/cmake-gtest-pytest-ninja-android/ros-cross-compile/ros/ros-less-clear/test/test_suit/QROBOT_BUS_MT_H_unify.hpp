...
// loop _topic_sub_threads (if topic==item.first)  and push msg 
       for(auto const& item : _topic_sub_threads) {
		...
		auto it_subs = _subs.find(std::get<0>(item)+std::get<1>(item));
			...
			}  else {std::cout<<"==BUS push msg("<< msg.frameid<<")===into==_subs:["<<std::get<0>(item)+std::get<1>(item)<<"] Queuesize="<<(it_subs->second)->safe_size() << "\n";     };	
           	(it_subs->second)->check_push(msg);  
		//#############dump_queue_qrobot_msg_type1(it_subs->second); 
         	} else {
			
				if (std::is_pointer<T>::value) {
					std::cout<<"==BUS NOT push msg("<< msg->frameid<<")===not found _subs:["<<std::get<0>(item)+std::get<1>(item)<<"]\n";
				} else {
					std::cout<<"==BUS NOT push msg("<< msg.frameid<<")===not found _subs:["<<std::get<0>(item)+std::get<1>(item)<<"]\n"; 
				};
...
//	std::cout << "*******************************************loop topic_sub_threads*************if topic==item.first*****  \n ";
			...
			if (std::is_pointer<T>::value) {
				std::cout<<"==BUS  NOT push msg("<< msg->frameid<<")===not found in _topic_sub_threads:["<< topic<<"]\n";		
			} else {
				std::cout<<"==BUS  NOT push msg("<< msg.frameid<<")===not found in _topic_sub_threads:["<< topic<<"]\n"; 
			};
