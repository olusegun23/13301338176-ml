...
bus<qrobot_msg_type3> mbus3;


utils tools;


std::vector<qrobot_msg_type1> g_algo1_out,g_algo1_in_1,g_algo2_in_1,g_algo3_in_1;
std::vector<qrobot_msg_type2> g_algo2_out,g_algo1_in_2,g_algo2_in_2,g_algo3_in_2;
std::vector<qrobot_msg_type3> g_algo3_out,g_algo1_in_3,g_algo2_in_3,g_algo3_in_3;

int g_runs=1000;
int g_runs_spinonce=100;
int g_sleep=30000;// us 
int g_buffer_wait=2; // s  wait for all queue clear when thread stop
int g_thread_wait=2; // s wait for all thread sync when start

int g_isblock=0;
int g_fetchnum=0;
int g_subs_queue_size=1000;
...
public:
  ...
  std::cout<<"\n" <<" TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg1.frameid<<"]" << std::endl;
	...
	void callback_1_3(qrobot_msg_type3& msg3)
     {
...
public:
   algo_task2(std::string str):TASK_NAME(str){};
	...
	void callback_2_1(qrobot_msg_type1& msg1)
     {
  std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg1.frameid<<"]" << std::endl;
        g_algo2_in_1.push_back(msg1);
     }
	
	void callback_2_2(qrobot_msg_type2& msg2)
     {
     std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg2.frameid<<"]" << std::endl;
        g_algo2_in_2.push_back(msg2);
     }

	void callback_2_3(qrobot_msg_type3& msg3)
     {
     std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg3.frameid<<"]" << std::endl;
   ...
   void operator() () {
	...
	int i=1;
	while (i<g_runs) {
        qrobot_msg_type2 msg2;
        msg2.frameid=i;
	msg2.map2d.push_back(std::vector<float>(4, 2.0));
	{
	// g_mut_log.lock();
...
public:
   algo_task3(std::string str):TASK_NAME(str){};
	...
	void callback_3_1(qrobot_msg_type1& msg1)
     {
  std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg1.frameid<<"]" << std::endl;
        g_algo3_in_1.push_back(msg1);
     }
	
	void callback_3_2(qrobot_msg_type2& msg2)
     {
     std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg2.frameid<<"]" << std::endl;
        g_algo3_in_2.push_back(msg2);
     }

	void callback_3_3(qrobot_msg_type3& msg3)
     {
     std::cout<<"\n" << "TASK_NAME("<< TASK_NAME<<").FUCTION(" << __FUNCTION__  << ")-msg.frameid["<<msg3.frameid<<"]" << std::endl;
