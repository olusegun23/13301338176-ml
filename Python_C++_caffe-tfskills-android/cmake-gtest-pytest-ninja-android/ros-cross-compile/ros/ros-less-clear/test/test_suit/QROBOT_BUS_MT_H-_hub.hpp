...
//int  subscribe(const std::string& threadname, const std::string& topic,const int queuesize,const int options,std::function<void(std::shared_ptr<T>)> callback,std::string& errorcode)  {
...
//        std::unique_lock<std::mutex> mlock_callbacks(_mutex_callbacks);    
	...
	//	std::function<void(std::shared_ptr<T>)> f=it_callback->second;
...
//std::map<std::string,std::function<void(std::shared_ptr<T>)>>  _callbacks;
