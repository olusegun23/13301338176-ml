#ifndef MAP2D_QROBOT_TASK_H
#define MAP2D_QROBOT_TASK_H
...
class  algo_2dmap:public qrobot_task {
public:
	algo_2dmap(std::string str):TASK_NAME(str){};

	template<typename T>
	void callback_2dmap_dfs(T msg1) {
	...
	void operator() () {
		...
		std::string error("   ok           ");
		mhub.advertise<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"2DMAP",2,0,error); 
 		std::function<void(std::shared_ptr<qrobot_msg_type2>)> f1=std::bind(&algo_2dmap::callback_2dmap_dfs<std::shared_ptr<qrobot_msg_type2>>,this,std::placeholders::_1);
		mhub.subscribe<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"DFS",g_subs_queue_size,0,f1,error);
	...
	//======================feed dynamic vector of vector data========================================
			...
			mhub.publish<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"2DMAP",msg2,0,error);

			if (i>g_fetchnum){ 
				mhub.spinOne<std::shared_ptr<qrobot_msg_type2>>(TASK_NAME,"DFS",g_fetchnum,g_isblock,error);
			};
